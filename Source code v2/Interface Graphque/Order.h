...
public:
	...
	void save_parameter() {
		...
		if (this->id_order == "") {
			...
			id_adresse = link::get_first_item(link::execute(request, "ADRESSE"));

			// get the last id from the table ACHAT
			request = "SELECT TOP 1 ACH_ID_ACHAT FROM ACHAT;";
			// increment the id
			id_achat = to_string(stoi(link::get_first_item(link::execute(request, "ACHAT"))) + 1);
			...
			for (int i = 0; i < item_number; i++) {
				...
				price = link::get_first_item(link::execute(request, "PRODUIT"));

				// get the item id
				request = "SELECT PRO_ID_PRODUIT FROM PRODUIT WHERE PRO_NOM_PRODUIT LIKE '" + item_name + "'";
				id_item = link::get_first_item(link::execute(request, "PRODUIT"));
			...
			string last_id = link::get_first_item(link::execute(request, "FACTURE"));
			// increment the id
			...
			response = link::get_first_item(link::execute(request, "FACTURE"));
			...
			response += "\n" + link::get_first_item(link::execute(request, "COMMANDE"));
		}
		else {
			// update the order
