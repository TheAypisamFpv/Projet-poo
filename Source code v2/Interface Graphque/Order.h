...
public:
	...
	void save_parameter() {
		...
		if (this->id_order == "") {
			...
			id_adresse = link::get_first_item(link::get(request, "ADRESSE"));

			// get the last id from the table ACHAT
			request = "SELECT TOP 1 ACH_ID_ACHAT FROM ACHAT;";
			// increment the id
			id_achat = to_string(stoi(link::get_first_item(link::get(request, "ACHAT"))) + 1);
			...
			string last_id = link::get_first_item(link::get(request, "FACTURE"));
			// increment the id
